...
export default function DesktopLayout({ children, activeView, onNavigate, isFullscreen = false }) {
  ...
  return (
    <div className={cn('w-full h-screen grid overflow-hidden', cols)}>
      ...
      {/*
        ...
        to the viewport so iPad can't drag it.
      */}
      <main className="h-screen overflow-y-auto overscroll-contain bg-[var(--ds-background-100)] flex flex-col relative w-full">
        {children}
      </main>
    </div>
  );
