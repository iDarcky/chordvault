...
export default function SectionBlock({
  ...
  const renderLine = (line, idx) => {
      ...
      if (line.type === 'modulate') {
        return (
          <div key={idx} className="my-6 flex items-center gap-4">
            <div className="h-[1px] flex-1 bg-[var(--color-primary-300)]" />
            <span className="text-label-10 font-black uppercase tracking-[0.2em] px-3 py-1 bg-primary text-white rounded-full shadow-sm">
    ...
    if (!cleanLine.includes('[') || !showChords) {
      ...
      return (
        <div key={idx} className="min-h-[1.5em] whitespace-pre-wrap text-foreground opacity-90">
          {displayLine}
          {inlineNotes && inlineNote && (
            <span className="text-default-500 italic text-[0.8em]">
    ...
    return (
      <div key={idx} className="mb-4 last:mb-0 overflow-x-auto">
        ...
        <div className="text-foreground whitespace-pre leading-tight">
          {lyricLine || ' '}
          {inlineNotes && inlineNote && (
            <span className="text-default-500 italic text-[0.8em]">
              {NOTE_SEPARATORS[noteStyle] || NOTE_SEPARATORS.dashes}{inlineNote}
    ...
    <div className="mb-8 md:mb-12 break-inside-avoid">
      <div className="flex items-center gap-4 mb-6">
        <div className="flex flex-col">
          ...
          {section.note && (
            <span className="text-label-11 italic text-default-500 mt-1 px-1 ml-0.5 border-l-2" style={{ borderColor: s.br }}>
              {section.note}
            </span>
          )}
        </div>
        <div className="h-[1px] flex-1 bg-[var(--color-default-200)] opacity-20" />
      </div>
