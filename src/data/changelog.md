...
- **The set across the top (Labs)** — the reader can show the app's original player bar above the song's title: a progress line for the whole service and every song as a chip you can tap, with breaks marked. It takes nothing from the song's own structure, which keeps its place below. Switch it on in the display menu.
...
- **Four ways to mark a section, none of them a box (Labs)** — None, Rule, a bar in the left margin, or a Tint that runs edge to edge. The old Block and Card frames drew a box around the words: on a phone a Card chorus spent 58px on padding before a lyric started, and its pinned heading sat as a dark slab inside a lighter card. Nothing takes width from the words any more. If you were on Block or Card you're on Tint.
...
- **The ☰ menu is three tabs — Style, Layout, Music (Labs)** — opening it used to land you on a list you then had to pick from. It now opens straight into a panel, and the other two are one tap away in either direction. Notes has moved out; it's going to live with the setlist instead.
- **On a phone the ☰ splits the screen instead of covering it (Labs)** — the song keeps the top 70%, the settings take the bottom 30%, and the chart is genuinely shorter rather than hidden behind a panel. It doesn't dim the song and it doesn't need dismissing: the ☰ turns into a ✕ while it's open, and the song above stays live, so you can still tap a chord for its shape while you change the type size.
- **The ☰ lights up while it's open (Labs)** — the same way the metronome button does, instead of turning into a ✕ that sat opposite the exit ✕ meaning something completely different. Pressing edit now closes the settings too, since editing takes the screen anyway.
...
- **The progress line can be turned off (Labs)** — the hairline at the very top showing how far through the service you are.
...
- **Switches sit on their own label's line (Labs)** — three of them stacked used to take up half the panel.
- **Settings pair up only when they fit (Labs)** — two side by side in a wide panel, stacked in a narrow one, instead of always squeezing into two columns.
- **The settings panel closes with a chevron on a phone (Labs)** — it slides down, so the arrow says which way it goes.
- **Smaller switches (Labs)**, now that they sit on their label's line.
...
- **Everything in the ☰ is bigger (Labs)** — the tabs, the buttons, the ± steppers, the colour dots, the theme tiles and the labels. It's read from a music stand at arm's length, not held at reading distance, and it was sized for the latter. The two different button styles it had picked up along the way are now one.
- **The themes are a carousel with arrows (Labs)** — so it's obvious there are more than the three you can see.
- **You can see the themes you don't have yet (Labs)** — the Pro themes used to be hidden entirely, so there was nothing to want. They're shown now, dimmed with a padlock, with one button to unlock them. Everything that makes a chart readable — every text size, line spacing and section gap — stays free.
- **The settings labels are readable (Labs)** — they were 10px monospaced capitals with wide letter-spacing, which is five ways of whispering at once. Now they're the app's normal type, a size up, in ordinary sentence case.
- **The selected chart theme has a clean outline (Labs)** — it was drawing two rings of the same colour at slightly different corner radii, which made the edge look furred. It's one ring now, matching the colour swatches below it.
- **Edit a song while you practise (Labs)** — a pencil beside the metronome turns the chart itself into the editor: the tempo and the time signature at the top become fields you type in, and every section grows arrows to move it in the play order and a × to take it out. Nothing covers the song while you change it, and the top bar turns **orange** so you always know you're editing. Undo steps back through your changes, Cancel puts everything back the way it was, and the exit is held until you've finished one way or the other. Practice only — not during a service. When you've changed something, one button keeps the edit as a **new arrangement** and restores the original.
- **Fix a chord without leaving the song (Labs)** — while you're editing in practice, tapping any chord opens the song editor's chord bar: type any chord you like (slash chords included) or tap one of the suggestions from the song's key. On a phone it sits across the bottom of the screen instead of floating, the suggestions wrap onto two short rows rather than scrolling sideways, and the empty strip above the close button is gone. The chord is written back in the song's own key, so a chart you're reading in D still saves correctly.
- **Editing a song holds still while you do it (Labs)** — the bottom navigation bar, the floating page arrows and the song counter all step out of the way instead of sitting on top of the editing bar, opening the editor closes the practice strip and greys out everything that would take you out of the song mid-change: the menu, the metronome, the setlist and the exit. Finish or discard first. Editing turns **the set across the top and the title bar** solid orange together, rather than one orange band sitting between two normal ones — a fixed pair of colours, so it reads exactly the same on every chart theme instead of tinting a different amount on each one. The song map underneath keeps the chart's own paper and its usual section colours, because that's the part you're reading while you work. Text no longer gets selected when you press and hold in the reader, so dragging works properly in the installed app.
- **Pull down to finish editing (Labs)** — on a phone, once you're at the top of the song and can't scroll any further, drag down and the chart lets go of edit mode. The header follows your thumb and tells you when to release. Works in the installed app, where there's no browser refresh to get in the way.
- **Edit a section's words (Labs)** — every section gets a pencil beside its bin while you're editing, with the same two modes the song editor has: **Lyrics**, where you fix the words and your chords stay attached, and **Source**, where you edit the line exactly as it's written. Sections holding a tab or a key change open straight into Source and say why.
- **Delete a section from the song map (Labs)** — while you're dragging a chip, a bin appears at the end of the row; drop it there. Same five-second Undo as the bin on the section itself.
- **The reader reaches the edge of the screen** — a strip reserved for a scrollbar was leaving a permanent line down the right-hand side of every full-screen view.
- **Dragging sections on the song map works (Labs)** — it never could: the code watching your finger was being torn down and rebuilt on every redraw, which threw the drag away the instant it started. Press and hold a chip and it moves now. The structure row also appears automatically when you start editing, even if you keep it hidden, and it wraps onto several lines instead of scrolling sideways so nothing fights your finger.
- **Undo a deleted section from the toast (Labs)** — taking a section out now shows the same five-second Undo you get elsewhere in the app.
- **Edit the play order on the song map (Labs)** — while editing, a **+** at the end of the structure row lets you pick any section and add it, and you can **press and hold a chip to drag it** somewhere else in the order, including past the last one. While you're editing, a chorus sung three times shows as three separate chips so you can move any one of them, and tapping a chip no longer jumps you down the page. The section list is colour-coded to match the map. The up/down arrows on the section headings are gone; a bin button to take a section out stays where you can see it.
- **The set's progress bar is always there (Labs)** — the thin line showing how far through the service you are used to belong to the set bar across the top, so turning that off took the progress with it. It now lives on the top bar itself, in the same place, whatever else is switched on.
- **The setlist sits beside the chart on a desktop or tablet (Labs)** — a slim strip down the right-hand edge that opens into the full running order and collapses back to a strip, with its own toggle. The chart never jumps when you open it, and it remembers how you left it on each device. The counter in the bottom bar still opens it too.
- **A smaller key chip in the reader**, so the top bar reads as one line rather than a row built around it.
- **The reader's top-bar icons sit together** as one group of tools instead of three loose buttons, which also gives the song title back some room — and everything in the header is a few pixels larger on a phone.
...
- **The metronome doesn't fall behind or lose its place.** Beat times are now computed from a fixed anchor rather than added up one at a time, so the click can't drift over a long song, and its timer runs off the main thread — where the browser can't slow it down while you're looking at something else. If it ever is interrupted, it comes back in on the right beat of the bar rather than restarting wherever the interruption ended.
- **The accent colour reaches everything now.** Setting one used to recolour buttons and focus rings while every soft fill, brand-coloured label and hairline stayed the original teal, so the app looked half-changed. All of them are derived from your accent now — including the reader's divider, which is a stronger tint so it reads as the colour you picked rather than as grey.
...
- **The 1 / 2 column choice is gone on phones**, where it never did anything: a phone always gets one column, so the switch was a control that quietly ignored you.
- **The line under the reader's title bar** runs the full width, carries your accent colour, and sits under the whole header — title row and structure together — rather than between them, so it reads as the edge of the chrome rather than an underline for the song title.
...
- **Section headings you can actually read (Labs)** — they were 12px against 18px lyrics, and smaller than the band cue riding on them. They're 14px now, 17px on a chorus, with the cue always a size below the name. The heading row also stopped reserving nine pixels of empty space per section: about 170px back on a ten-section song.
- **A chorus finally has air above it (Labs)** — the extra space was worth 1.6 pixels, and disappeared entirely if you raised Section spacing. It's real space now, above the section rather than below it, at every spacing.
- **The words start at the edge of a phone (Labs)** — the chart kept 32px down each side, 16% of a 390px screen spent on nothing. It's 12px now, which gains 40px of line and takes 20px off the song's height. The side the structure dots float down keeps its margin so they still cross no words.
- **The right side is for notes (Labs)** — an inline note sits in its own strip down the right, on the same line as the words it belongs to, instead of on a long dotted rule or on a line of its own. Only sections that actually have a note reserve the strip, so nothing else gets narrower. Notes used to sit level with the chord row rather than the lyric; they land on the line now.
- **Pinned headings work on a tablet and a desktop (Labs)** — the switch said On and did nothing on any screen 768px or wider, whatever the column count. It's real everywhere at one column, and the switch steps out of the way at two, where two headings would pin side by side.
...
- **The dots down the side stopped shuddering while you scroll (Labs)** — the one you're on grows a little, and it was pushing the dots below it down as it moved from section to section. They hold their places now.
- **The dots had a dashed line inside them in Firefox (Labs)** — Firefox draws a tapped button's focus ring inside the shape, which on a small dot is a dashed circle through the middle of it. Fixed both ways it can happen.
...
- **The song structure down the side, rebuilt (Labs)** — it sits in the middle of the screen instead of stacking from the top, shows six sections at a time and walks with you as you scroll instead of standing still, sits on a soft translucent plate so the lyrics show through without the chips going pale, spells repeats out instead of counting them ×2, and gets pushed aside by the settings panel and the setlist rail instead of ending up underneath them.
...
- **Pressing edit no longer hides the section you were reading (Labs)** — turning on the song map made the reader scroll down by the height of the map it had just added, which pushed the pinned section heading up behind it until you scrolled back. The scroll was meant to compensate for the map appearing; the page already did that on its own, so the "fix" was the bug.
...
- **You can type a space in a band cue** — every keystroke went out to the file and back, and the trip trimmed the space off the end, so you could type one word into a cue and no more. Band cues are capped at 70 characters now and inline notes at 40 — long enough for an instruction, short enough that the song stays on screen.
